	...
	if argstr:
		...
		if fcn=='push_string':
			if args[0].startswith('MSGID::'):
				msgid = args[0][7:]
				e = po.find_by_prefix("{}:".format(msgid))
				if e is not None and e.msgstr != "" and not e.msgstr.startswith("NAME:"):
					args[1] = '"{}"'.format(e.msgstr)
				del args[0]
...
def parse(asmtxt, po):
	...
	for id, line in enumerate(asmtxt.split('\n')):
		...
		if asdis.re_header.match(line):
			...
			fcn, args, strings = parse_instr(line, id+1, po)	
...
def out_hdr(fo, hdrtext, defines, symbols):
	fo.write(hdrtext.encode(buriko_setup.ienc).ljust(0x1C, b'\x00'))
	entries = len(defines)
	...
	for name in sorted(defines, key = lambda x: symbols[x]):
		fo.write(name.encode(buriko_setup.ienc) + b'\x00')
...
def out(fo, instrs, symbols, texts, hdrtext, defines):
	...
	for text in texts:
		fo.write(text.encode(buriko_setup.ienc) + b'\x00')

def asm(file):
	buriko_common.makedir('{}/compiled'.format(buriko_setup.project_name))
	scriptname = os.path.splitext(os.path.basename(script))[0]
	ofile = '{}/compiled/{}'.format(buriko_setup.project_name, scriptname)
	po = polib.pofile("{}/{}/{}.po".format(buriko_setup.project_name, scriptname, buriko_setup.ilang), klass=bgi_po.IndexedPo)
	asmtxt = open(file, 'r', encoding='utf-8-sig').read()
	instrs, symbols, texts, hdrtext, defines = parse(asmtxt, po)
